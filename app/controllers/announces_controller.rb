class AnnouncesController < ApplicationController
  before_action :set_announce, only: [:show, :edit, :deliver, :update, :destroy]
  ...
  def deliver
    @announce.users.each do |user|
      AnnounceJob.perform_later(@announce.id, user.id)
    end
    redirect_to @announce, notice: 'Announce was successfully delivered.'
  end
